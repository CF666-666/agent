...
| R0-E | 简历证据化：受控基线归档 | 🟡 阻塞 | 08-12 | - | schema v3 原始/合并报告的交叉校验归档器及 27 项离线回归已完成；真实四场景运行因百炼 Chat/Rerank 返回 `Arrearage` 且启动期 FAQ 入库未完成而未归档。恢复模型额度和稳定索引后按 `r0-e-baseline-blocker.md` 收口。 |
| R0-F | 运行环境：默认 SiliconFlow 模型路由 | 🟡 容器验收待补 | 08-12 | - | Chat、Embedding、Rerank、Vision 与 RAGAS 默认使用 SiliconFlow；保留百炼适配器，通过模型候选配置切换。请求契约、配置注入和本地 Maven 构建均通过；Docker 容器内 Maven 构建持续无输出超过 6 分钟，已停止构建客户端且未影响现有健康服务，待网络/构建缓存恢复后重建镜像验收。 |
