...
| R0-A | 简历证据化：移除 GitHub 展示 | ✅ 完成 | 08-12 | 08-12 | 普通 Header 与管理后台 Header 均移除 GitHub 图标、Star 数、仓库链接和 GitHub API 请求；前端生产构建与源码残留检查通过。后续计划见 `resume-evidence-development-plan.md`。 |
| R0-B | 简历证据化：评测数据契约 | ✅ 完成 | 08-12 | 08-12 | 新增零依赖 JSONL 契约，覆盖单轮、多轮、图像、关系与 RAGAS 分组字段；检索 Runner 加载前强制校验，兼容历史 v2 基线。19 项回归、v2 数据校验与确定性生成检查通过。 |
| R0-C | 简历证据化：执行指纹 | ✅ 完成 | 08-12 | 08-12 | 检索/RAGAS 报告新增无密钥执行指纹：运行器、数据契约、运行档案、应用配置、Git 提交与 Python 版本；检索报告升级 schema v3，合并器拒绝不同指纹和历史 v2 混合。21 项回归通过。 |
| R0-D | 简历证据化：调优/冻结集隔离 | ✅ 完成 | 08-12 | 08-12 | 新增跨数据集来源隔离校验：显式 `tuning/frozen` Schema、case type 与 split；拒绝共享 `golden_source_ids`、来源记录、图像路径或超边 ID，历史 legacy 基线不可参与隔离校验。25 项契约/评测回归、v2 校验与确定性生成检查通过。 |
| R0-E | 简历证据化：受控执行归档 | ✅ 完成 | 08-12 | 08-13 | schema v3 四场景运行、交叉校验与归档已完成；稳定索引实测 FAQ `210/210/210`、图像 12 条。旧 100 条运行因超图 LLM 实体抽取超预算而无质量样本，作为降级执行证据保留而非质量基线；超图根因已由 R2 关闭，最终四场景质量复验归入 R5。 |
| R0-E-1 | 简历证据化：检索时限与取消传播 | ✅ 完成 | 08-13 | 08-13 | 请求入口统一单调时钟 deadline；改写、意图、四路检索及 MCP 子任务共享剩余预算并可取消。超图通道独立 7 秒预算，SSE 新增 `retrieval_status` 保留通道级状态；评测将执行失败与质量样本分离。定向单测、真实 SSE 事件顺序和 HTTP 取消回归均通过；后续四场景归档已完成，但暴露出超图查询可用性瓶颈。 |
| R0-F | 运行环境：默认 SiliconFlow 模型路由 | ✅ 完成 | 08-12 | 08-12 | Chat、Embedding、Rerank、Vision 与 RAGAS 默认使用 SiliconFlow；保留百炼适配器，通过模型候选配置切换。新增 Docker Maven 镜像配置以规避 Maven Central TLS 握手故障；新后端镜像构建、容器重建、健康检查及登录接口均已验收。 |
| R1-A | PDF 智能解析：页级策略 seam | ✅ 完成 | 08-13 | 08-13 | 新增显式版本化阈值、`AUTO/PDFBOX/OCR` 模式、电子/扫描/空白/短文本/替换符与 mojibake 乱码决策和有序类型化页级结果；11 项新增行为测试及 2 项既有 PDFBox 回归通过，尚未接入 PDFBox 逐页分析或 OCR。 |
| R1-B | PDF 智能解析：PDFBox 逐页分析 | ✅ 完成 | 08-13 | 08-13 | 不依赖扩展名按 PDF 内容加载，逐页输出原生文本、字符数和图片视觉覆盖率；覆盖率按图片几何并集并尊重 CropBox 与活动裁剪路径，4 项结构集成测试通过。 |
| R1-C | PDF 智能解析：页级 OCR | ✅ 完成 | 08-13 | 08-13 | AUTO 只渲染/OCR 策略命中页，`PDFBOX/OCR` 强制模式不做隐式回退；Tess4J 接入页级端口，未就绪、异常或空结果均带页码使整文档失败，页图在调用后释放；R1 聚合 20 项测试通过。 |
| R1-D | PDF 智能解析：入库与页级证据闭环 | ✅ 完成 | 08-13 | 08-13 | 多模态节点按 MIME 接入页级解析；类型化页结果进入检查点与节点输出，Chunk 保留页码/策略/原因和稳定 ID；OCR 失败不发布部分结果，无法保留页码映射的全文增强显式失败；真实混合 PDF 链路及 R1 聚合 28 项测试通过，R1 收口。 |
| R2-A | 超图关系质量：查询可用性恢复 | ✅ 完成 | 08-13 | 08-13 | 高置信索引实体/别名本地优先，零命中且预算充足才调用 LLM；不可变提及快照避免持读锁扫描，扫描与 1～2 跳遍历响应取消；预算不足标记 `DEGRADED` 并从质量计分排除；Java 20 项、Python 5 项测试通过，独立复审无 P0/P1。 |
| R2-B | 超图关系质量：冻结基线与失败分类 | ✅ 完成 | 08-13 | 08-13 | 固定 50 条关系集，tuning/frozen 各 25 条且证据隔离；SSE 保留逐超边/source 证据，50/50 可计分，逐超边 Hit@1/5=74%/82%、Recall@5=77%、严格有序路径=30%、逐边来源=74%；分类不从超时或业务标签推断因果，融合组仅作观察。 |
| R2-C1 | 超图关系质量：实体表面归一化 | ✅ 完成 | 08-13 | 08-13 | NFKC、大小写、空白与 `µ/μ` 单位统一；仅按 tuning 增加 4 个简称并修正 Spring 中文 Map key 绑定。20 项 Java 测试和独立复审通过；tuning 25 条观察归档，未运行 frozen，不宣称严格提升。 |
| R2-C2 | 超图关系质量：来源字段可用性契约 | ✅ 完成 | 08-13 | 08-13 | SSE 保留 6 类真实证据字段并逐字段标记 available/unavailable；评测按唯一超边统计缺失和冲突，禁止伪造 chunk/page。Demo 603 条来源审计、Java 11 项、Python 49 项及隔离 SSE 探针通过。 |
| R2-D | 超图关系质量：路径评分与冻结验收 | ✅ 完成 | 08-13 | 08-13 | 查询实体覆盖优先、短路径优先、类型权重与稳定 edge key 排序；唯一 frozen 23/25 可计分，Recall@5=100%、严格路径=91.30%、来源=100%，2 条通道超时单列；阶段门槛通过。 |
| R3-A | 查询鲁棒性：40 条单轮噪声难例 | ✅ 完成 | 08-13 | 08-13 | 错词/同音、同义/简称、单位/格式、口语省略/语序各 10 条；tuning（钢铁）/frozen（电力）来源隔离，规范问题、变体说明与 FAQ 记录可追溯。 |
| R3-B | 查询鲁棒性：20 组多轮会话 Runner | ✅ 完成 | 08-13 | 08-13 | 10 组省略、10 组跨轮指代，2/3 轮各 10 组；Runner 完整消费真实回答、回读确认历史持久化并复用服务端会话 ID，只评分目标轮；61 项评测脚本测试通过。 |
| R3-C | 查询鲁棒性：冻结集严格 rewrite A/B | ✅ 完成 | 08-13 | 08-13 | 前置根因：索引仅 77/210 条（电力 0 条）→ 按 qwen-emb-8b 全量重建 210 条。frozen 会话集公共配对 9 条：Hit@1 0.50→1.00、MRR +0.4074（相对 +84.6%）；noise 20 条 Hit@1 0%→45%；on 执行成功率 90%、目标轮 P50 约 40s（off 约 26s），限制性披露见 r3c 报告 README。 |
