...
| R0-E | 简历证据化：受控基线归档 | 🟡 进行中 | 08-12 | - | schema v3 原始/合并报告交叉校验归档器及离线回归已完成；启动期 FAQ 入库已改为固定模型、有界重试、失败清理和强一致性三方计数校验，默认模型路由为 SiliconFlow。2026-08-12 已实测 FAQ `210/210/210`、图像 12 条；待稳定索引下完成真实四场景运行与归档，详见 `r0-e-baseline-blocker.md`。|
| R0-E-1 | 简历证据化：检索时限与取消传播 | ✅ 完成 | 08-13 | 08-13 | 请求入口统一单调时钟 deadline；改写、意图、四路检索及 MCP 子任务共享剩余预算并可取消。超图通道独立 7 秒预算，SSE 新增 `retrieval_status` 保留通道级状态；评测将执行失败与质量样本分离。定向单测、真实 SSE 事件顺序和 HTTP 取消回归均通过；R0-E 的稳定索引四场景归档仍待完成。 |
