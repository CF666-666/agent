...
| R0-E | 简历证据化：受控基线归档 | ⚠️ 执行闭环完成，质量门禁阻塞 | 08-12 | 08-13 | schema v3 四场景运行、交叉校验与归档已完成；稳定索引实测 FAQ `210/210/210`、图像 12 条。真实 100 条全链路运行中，100/100 因超图 LLM 实体抽取超过 7 秒通道预算而降级，P50/P95=12.917/14.570s，无可计分质量样本；归档见 `scripts/eval/report/baselines/r0e-20260813-degraded-full-chain/`。须先修复超图查询可用性，不能将本次结果用于简历指标。|
