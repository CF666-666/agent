...
| R0-E | 简历证据化：受控基线归档 | 🟡 进行中 | 08-12 | - | schema v3 原始/合并报告交叉校验归档器及离线回归已完成；启动期 FAQ 入库已改为固定模型、有界重试、失败清理和三方计数校验，默认模型路由为 SiliconFlow。待稳定索引下完成真实四场景运行与归档，详见 `r0-e-baseline-blocker.md`。|
