...
结论：本次结果证明请求 deadline、通道降级、状态事件和归档链路可用，但**不构成当前全链路检索质量基线，也不能用于简历量化陈述**。该超图查询阻塞已在 R2 中通过本地实体优先、受控 LLM 兜底与路径评分关闭；R0 因此按“评测基础设施与阻塞诊断完成”收口。当前版本不在 R0 重复运行旧 100 条集，四场景最终质量复验将在 R3/R4 完成后归入 R5 统一执行。
