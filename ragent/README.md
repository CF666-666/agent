...
- **多模态文档理解**：文本 + 图表 + 图像的联合解析，支持 PDF / Word / Markdown / 图纸扫描件，通过 OCR 与版面分析把"一张图纸"变成可检索的结构化知识。

- **多路检索引擎**：意图定向检索 + 全局向量检索并行执行，结果经去重、重排序后处理，兼顾精准度与召回率。

- **图像检索链**：设备图纸、表格图表走独立的视觉检索通道（多模态 Embedding + 图像向量集合），回答问题时能"按图索骥"，直接召回并渲染原始图纸。

- **超图推理引擎**：知识点 → 超边建模，跨文档、跨实体建立多跳关系（633 条超边），针对复杂工业问题做关系路径推理，弥补纯向量检索"只认相似、不懂关系"的短板。
...
- **多模态来源渲染**：回答末尾以卡片形式展示"文本引用 / 图纸大图 / 推理路径 / 来源过滤"，答案可溯源、可验证。
- **用户中心**：注册登录、个人资料管理、头像上传（本地落盘 + 内置默认头像）、管理后台（知识库 / 用户 / 链路追踪）。
...
- [快速开始](#快速开始)
    - [方式一：一键 Docker Compose 启动](#方式一一键-docker-compose-启动)
...
| 前端框架   | React 18、Vite、TypeScript、Tailwind、Radix UI               |
| 关系数据库 | PostgreSQL + pgvector（20 多张业务表）                       |
| 向量数据库 | Milvus 2.6（文本 / 图像双集合）                              |
| 缓存/限流  | Redis + Redisson                                             |
| 对象存储   | S3 兼容存储（RustFS），头像走本地磁盘静态资源                 |
| 消息队列   | RocketMQ 5.x                                                 |
| 文档解析   | Apache Tika 3.2 + OCR（Tesseract）                           |
| 多模态 Embedding | Qwen3-Embedding-8B、图文联合向量化                           |
...
在纯文本 RAG 的基础上，Ragent 面向工业/制造场景完成了多模态升级：让系统"看得懂图纸、查得到关系、答得出依据"。

```mermaid
flowchart LR
    subgraph 数据层
        A1[PDF / Word / 扫描件] --> A2[Tika + OCR 解析]
        A3[设备图纸 / 图表] --> A4[图像解析与增强]
    end
    ...
    subgraph 检索层
        Q1[用户问题] --> D1[查询改写 + 意图路由]
        D1 --> E1[文本通道检索]
        D1 --> E2[图像通道检索]
        D1 --> E3[超图关系推理]
        E1 & E2 & E3 --> F1[多路融合 + Rerank]
    end

    F1 --> G1[LLM 答案生成]
    G1 --> H1[references 多模态渲染<br/>文本引用卡 / 图纸 Lightbox / 推理路径]
```

关键点：

- **图像检索链**：图纸按视觉特征向量化后进入独立的图像集合，用户提问"这个零件怎么装配"时，系统不仅能给出文字步骤，还能把对应的原始图纸作为图片证据一并召回展示。
- **超图推理引擎**：以"知识点 → 超边"的方式建模跨文档关系（如"油泵"→"密封圈"→"装配扭矩"），当问题需要多跳推理时，超图路径检索补足向量检索的盲区。
- **多路融合**：文本、图像、超图三路结果按置信度加权融合，经 Rerank 后进入生成环节，同时把每条引用的来源类型（文本/图像/路径）结构化传给前端。
- **references 渲染**：前端按来源类型分别渲染——文本引用卡片、可放大的图纸 Lightbox、推理路径拓扑，让答案"所见即所得、所得可溯源"。
...
Ragent 支持两种启动方式：**一键容器化启动**（推荐，零依赖，冷开机即可演示）与**本地开发模式**（适合二次开发）。

### 方式一：一键 Docker Compose 启动
...
```bash
# 1. 先启动中间件（PostgreSQL / Redis / RocketMQ / Milvus）
...
# 3. 启动前端（开发服务器，端口 5173，自动代理 /api 到后端）
cd ragent/frontend
npm install
npm run dev
```
...
- [📚 文档总索引](../DOCUMENTATION.md) — 全部文档按用途分类导航（含未升级原始版）
...
- [未升级原始文档](README-v1.0.md) — 升级前 v1.0 版本项目介绍
